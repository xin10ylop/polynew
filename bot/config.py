"""Bot configuration (env-overridable). Defaults = the variant that survived out-of-sample testing on the
recent regime ('b3_g300': 3 ticks behind the best bid, BTC guard 300 ms / 0.3 bp, 10-share clips). See REPORT.md §4c."""
import os
...
class Config:
    ...
    back_ticks: int = _f("BOT_BACK_TICKS", 3)      # quote this many ticks behind the best bid on each side
    ...
    guard_window_ms: int = _f("BOT_GUARD_W_MS", 300)    # BTC lead-move guard window
    guard_bp: float = _f("BOT_GUARD_BP", 0.3)            # pull the stale side if BTC moves >= this many bp within window
    guard_cool_ms: int = _f("BOT_GUARD_COOL_MS", 1500)   # keep that side pulled this long
