...
SIDES = a[5].split(",") if len(a) > 5 else ["YES", "NO"]
MIN_DAYS = float(a[6]) if len(a) > 6 else 0.0
S = pd.read_parquet("data/slow/hit_weekly_samples.parquet").sort_values("t")
EV = pd.read_parquet("data/slow/events_hit_weekly.parquet").reset_index(drop=True)
S["days_left"] = (EV.loc[S.m, "end"].values - S.t.values) / 86400
q = S.qt1440.values
...
T = S[S.side.isin(SIDES) & (S.days_left >= MIN_DAYS)].copy()
pos = {}
fills = []
...
print(f"sides {SIDES}, min days left {MIN_DAYS}, thr {THR}, half spread {100 * HS:.1f}c, slippage {100 * SLIP:.1f}c, clip ${CLIP:.0f}, max ${MAXPOS:.0f}/strike")
...
W["end"] = pd.to_datetime(W.index.map(EV.groupby("event").end.max()), unit="s")
for mo in sorted(W.end.dt.strftime("%Y-%m").unique())[-4:]:
    w = W[W.end.dt.strftime("%Y-%m") == mo]
    print(f"  {mo}: weeks {len(w)}, PnL ${w.pnl.sum():+,.0f} ({' '.join(f'{p:+.0f}' for p in w.pnl)})")
